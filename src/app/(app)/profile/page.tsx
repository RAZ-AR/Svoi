    ...
    <div className="flex min-h-screen flex-col bg-[#EBEBEB]">
      ...
      <div className="mx-4 mt-2 overflow-hidden rounded-3xl bg-white">
        <div className="flex items-center gap-4 p-5">
          <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full bg-[#E0E0E0]">
            {user.avatar_url ? (
              <Image
                src={user.avatar_url}
                alt={user.first_name}
              ...
              <div className="flex h-full w-full items-center justify-center text-2xl font-bold text-[#999999]">
          ...
          <div className="min-w-0 flex-1">
            ...
            {user.telegram_username && (
              <p className="text-sm text-[#999999]">@{user.telegram_username}</p>
            )}
            {user.location && (
              <p className="mt-0.5 text-xs text-[#999999]">📍 {user.location}</p>
            )}
          </div>

          <Link
            href="/profile/edit"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-[#EBEBEB] transition-colors active:bg-[#E0E0E0]"
          >
            <Settings size={18} className="text-[#555555]" />
          </Link>
        </div>
      ...
      <div className="mx-4 mt-4 overflow-hidden rounded-3xl bg-white shadow-sm">
        <div className="flex items-center gap-3 px-4 py-4">
          <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#E0E0E0] text-[#555555]">
            <Globe size={18} />
          </span>
      ...
      <div className="mx-4 mt-4 overflow-hidden rounded-3xl bg-white shadow-sm">
        <button
          ...
          className="flex w-full items-center gap-3 px-4 py-4 transition-colors active:bg-[#EBEBEB]"
        >
...
function MenuRow({
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-3 px-4 py-4 transition-colors active:bg-[#EBEBEB]"
    >
